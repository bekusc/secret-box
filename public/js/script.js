...
$form_submit.click(function() {
	...
	if (!$(this).hasClass('disabled')) {
		...
		if ($age > 80 || $form_message.val().length < 50 || ($gender != 1 && $gender != 2)) {
			$('#error-zone').show();
		}
		else {
			$secret_form.hide();
...
$search_submit.click(function() {
	...
	var query = $search_form.find('#search-query').val(),
		age = $search_form.find('#age').val(),
		gender = 3;

	if ($gender.size() == 1) gender = $gender.data("value");
	if (!age) age = 1;

	window.location.href = "/search/" + query + "/" + age + "/" + gender + "/1";
});
...
$show_comments.click(function() {
	...
	if (!$comments_box.hasClass("loaded")) {
		...
		$comments_box.addClass("loading").append('<div class="fb-comments" data-href="http://beku.dev:8888/seg/' + title + '" data-width="600" data-numposts="10"></div>');
